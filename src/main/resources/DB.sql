...
CREATE UNIQUE INDEX IF NOT EXISTS user_login_index ON t_user (login);
...
CREATE TABLE IF NOT EXISTS t_chat_user (
    ...
    CONSTRAINT t_chat_user_pk PRIMARY KEY (chat_id, user_id),
    CONSTRAINT t_chat_fk      FOREIGN KEY (chat_id) REFERENCES t_chat (id),
    CONSTRAINT t_user_fk      FOREIGN KEY (user_id) REFERENCES t_user (id)
);
